  ...
  ipv4Lookup().then((_host) => {
    ...
    app.prepare().then(() => {
      ...
      server.listen(port, host, () => {
        ...
        deviceFlowUI.signIn().then((user) => {
          ui.succeed(`Firebase Auth: Signed in as ${user.email} (${user.uid})`);
          ...
          findController(argv.simulator || false).then(controller => {
            controller.start((messages) =>  {
              ...
              messages.forEach((msg) => {
                ...
                appendFileSync(`logs/${argv.simulator ? 'simulator_' : ''}${(new Date()).toISOString().split('T')[0]}.txt`, JSON.stringify(msg) + '\n');
          ...
          ui.fail(`Firebase Auth Error: ${err}`);
        });
      });
    });
  });
